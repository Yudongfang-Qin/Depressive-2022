...
##############################################################################################################
epochs_num = 1000
y_name = 'state_clinical_anxiety'
# [state_clinical_anxiety, state_high_anxiety, trait_all_future_hospitalizations]
version = str(4)
learn_rate = 0.00001
# y_name = 'state_clinical_anxiety'
# y_name = 'state_clinical_anxiety'
...
#
X_test=np.load("data/data_without_v123/X.npy")
y_test=np.load("data/data_without_v123/"+y_name+".npy")
X_train=np.load("data/data_v123/X.npy")
y_train=np.load("data/data_v123/"+y_name+".npy")
...
model.compile(loss='binary_crossentropy', optimizer=keras.optimizers.Adam(learning_rate=learn_rate), metrics=['accuracy','AUC'])
...
plt.title(y_name+'_v123')
plt.savefig('data//'+y_name+'_accuracy.png', dpi=600)
plt.show()
...
plt.title(y_name+'_v123')
plt.savefig('data//'+y_name+'_auc.png', dpi=600)
plt.show()

np.save('data//acc_tr_'+version+'.npy',history.history['accuracy'])
np.save('data//auc_tr_'+version+'.npy',history.history['auc'])
np.save('data//acc_te_'+version+'.npy',history.history['val_accuracy'])
np.save('data//auc_te_'+version+'.npy',history.history['val_auc'])

model.save('data_1//'+y_name+'_'+version+'.h5')
...
confusion_matrix_0['Sensitivity '] = confusion_matrix_0['TP'] / (confusion_matrix_0['TP'] + confusion_matrix_0['FN'])
confusion_matrix_0['Specificity'] = confusion_matrix_0['TN'] / (confusion_matrix_0['FP'] + confusion_matrix_0['TN'])
