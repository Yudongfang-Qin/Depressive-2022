...
df=df.iloc[np.asarray(~np.isnan(df.iloc[:,  16+correction])),:]
...
np.save(direction+'/data_number_of_all_future'+file_name+'/X', X)
np.save(direction+'/data_number_of_all_future'+file_name+'/X_reduced', X_reduced)
print(X[0:10])
